...
async function analyzeImageService(imageBase64: string, description?: string) {
  try {
    ...
    const systemInstruction = await getImageAnalysisInstruction();
...
export async function generatePromptFromImageService(
  ...
  try {
    // Step 1: Analyze the image and get structured data directly
    const parsedData = await analyzeImageService(imageBase64, description);

    // Step 2: Map snake_case to camelCase for frontend
    const mappedData = {
      coreTheme: parsedData.promptAttributes.core_theme,
      hair: parsedData.promptAttributes.hair,
      pose: parsedData.promptAttributes.pose,
